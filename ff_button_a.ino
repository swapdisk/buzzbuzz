...
#define BUTTON_PIN 6
#define MESSAGE "a"
#define DEVICE_NAME "FF Buzzer A"
#define STATUS_LED 7

// Debounce and battery report timers
unsigned long debounceTime = 3000;
unsigned long lastBuzz = debounceTime;
unsigned long battReportTime = 15000;
unsigned long lastBattReport = battReportTime;
unsigned long nowTime;

// Return battery level as percent
int battLevel() {
  int battPct = (analogReadMilliVolts(0) - 1850) * 100 / 250;
  return ((battPct < 0) ? 0 : (battPct > 100) ? 100 : battPct);
}
...
void loop() {
  ...
  if (isBleConnected && digitalRead(BUTTON_PIN) == LOW) {
    if (nowTime - lastBuzz > debounceTime) {
      // Debug output
      Serial.print("Sending keypress at ");
  ...
  // Status blink logic
  // Slow short blink: Bluetooth connected
  // Quick long blink: Waiting for connection
  if (isBleConnected) {
    digitalWrite(STATUS_LED, (nowTime % 1500 > 100) ? LOW : HIGH);
  } else {
    digitalWrite(STATUS_LED, (nowTime % 500 > 400) ? LOW : HIGH);
  }
}
...
void bluetoothTask(void*) {
    ...
    while(true) {
      ...
      int anamv = analogReadMilliVolts(0);

      // Debug output
      Serial.print("Battery % ");
      Serial.print(bl);
      Serial.print(", anaread/mv ");
      Serial.print(anaread);
      Serial.print("/");
      Serial.print(anamv);
      Serial.print(", diff ");
      Serial.print(anamv - anaread);
      Serial.print(", mv ");
      Serial.println(anamv * 2);

      // Update battery level
      hid->setBatteryLevel(bl);

      delay(battReportTime);
    }
...
void typeKustom(const char* text) {
    int len = strlen(text);
    for (int i = 0; i < len; i++) {
